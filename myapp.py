...
if file:
    ...
    with st.expander('Value Count'):
        ...
        if count:
            ...
            fig = px.bar(data_frame=result, x=column, y=result.iloc[:,1],text=result.iloc[:,1], template='plotly_white')
            st.plotly_chart(fig)
            fig = px.line(data_frame=result, x=column, y=result.iloc[:,1],text=result.iloc[:,1], template='plotly_white')
            st.plotly_chart(fig)
            fig = px.pie(data_frame=result, names=column, values=result.iloc[:,1])
            st.plotly_chart(fig)
    ...
    with st.expander('Group By your columns'):
        ...
        with col2:
            operation_col=st.selectbox('Choose columns for operatin',options=list(data.columns))
        with col3:
            operation=st.selectbox('Choose operatin',options=['sum','max','min','mean','median','count'])
        if(groupby_cols):
